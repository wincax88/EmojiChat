...
- (AGEmojiKeyboardViewCategoryImage)defaultSelectedCategory {
    ...
    NSNumber *category = [[NSUserDefaults standardUserDefaults] objectForKey:RecentUsedEmojiCategoryKey];
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
    ...
    if (self) {
        ...
        //self.backgroundColor = AGEmojiKeyBoardTinColor;
        // initialize category
...
- (void)setSelectedCategoryImageInSegmentControl:(UISegmentedControl *)segmentsBar
    ...
    for (int i=0; i < self.segmentsBar.numberOfSegments; ++i) {
        ...
        /*
        if ([[segmentsBar.subviews objectAtIndex:i] respondsToSelector:@selector(isSelected)] && [[segmentsBar.subviews objectAtIndex:i]isSelected])
        {
            [[segmentsBar.subviews objectAtIndex:i] setTintColor:[UIColor grayColor]];
        }
        if ([[segmentsBar.subviews objectAtIndex:i] respondsToSelector:@selector(isSelected)] && ![[segmentsBar.subviews objectAtIndex:i] isSelected])
        {
            [[segmentsBar.subviews objectAtIndex:i] setTintColor:AGEmojiKeyBoardTinColor];
        }*/
    }
...
- (void)categoryChangedViaSegmentsBar:(UISegmentedControl *)sender {
    ...
    // store current page
    [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInteger:sender.selectedSegmentIndex] forKey:RecentUsedEmojiCategoryKey];
