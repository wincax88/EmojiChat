...
- (void)setup {
    ...
    _bottomBar.color = [UIColor clearColor];
    _bottomBar.backgroundColor = [UIColor clearColor];
    _bottomBar.alpha = 0.f;
    [self addSubview:_bottomBar];
    
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 30, 1024, _barHeight)];
    _bottomView.alpha = 1.0;
    _bottomView.backgroundColor = [UIColor whiteColor];
    [_bottomBar addSubview:_bottomView];
    ...
    _timeElapsedLabel.textColor = [UIColor blackColor];
    ...
    _timeRemainingLabel.textColor = [UIColor blackColor];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        [_fullscreenButton setImage:[UIImage imageNamed:@"back_blue.png"] forState:UIControlStateNormal];
        ...
        [_fullscreenButton addTarget:self action:@selector(cancelMP:/*fullscreenPressed:*/) forControlEvents:UIControlEventTouchUpInside];
        ...
        //_volumeView.backgroundColor = [UIColor lightGrayColor];
        ...
        //[_bottomBar addSubview:volumeSlider];
        
        _seekForwardButton = [[ALButton alloc] init];
        [_seekForwardButton setImage:[UIImage imageNamed:@"play_forward_blue.png"] forState:UIControlStateNormal];
        [_seekForwardButton setImage:[UIImage imageNamed:@"play_forward_blue.png"] forState:UIControlStateSelected];
        ...
        [_shareButton setImage:[UIImage imageNamed:@"share_blue.png"] forState:UIControlStateNormal];
        [_shareButton setImage:[UIImage imageNamed:@"share_blue.png"] forState:UIControlStateSelected];
    ...
    [_playPauseButton setImage:[UIImage imageNamed:@"play_blue.png"] forState:UIControlStateSelected];
...
- (void)setBarColor:(UIColor *)barColor {
    if (_barColor != barColor) {
        ...
        [self.bottomBar setColor:[UIColor clearColor]];
    }
}
...
- (void)cancelMP:(UIButton*)button {
    
    //[self stopDurationTimer];
    [self.moviePlayer stop];
    [[NSNotificationCenter defaultCenter] removeObserver:self.moviePlayer name:MPMoviePlayerLoadStateDidChangeNotification object:nil];
    [self.moviePlayer.delegate backToPreviousViewController];
}

- (void)sharePressed:(UIButton*)button {

    [self.delegate shareToOthers:button];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.style == ALMoviePlayerControlsStyleNone)
        return;
    self.isShowing ? /*[self hideControls:nil]*/[self hideControls:nil]: [self showControls:nil];
}
...
- (void)movieFinished:(NSNotification *)note {
    ...
    if (self.moviePlayer.currentPlaybackTime >= self.moviePlayer.duration - 1) {
        isPlayEnd = YES;
    }
    ...
    if (!self.fullscreenButton.touchInside && isPlayEnd == YES) {
        [self.moviePlayer movieTimedOut];
    }
}
...
- (void)monitorMoviePlayback {
    ...
    if (self.moviePlayer.currentPlaybackTime <= self.moviePlayer.duration && self.moviePlayer.currentPlaybackTime >= 0.0) {
        currentTime = floor(self.moviePlayer.currentPlaybackTime);
    }else {
        currentTime = 0.0;
    }
    double totalTime = floor(self.moviePlayer.duration);
...
- (void)layoutSubviews {
    ...
    CGFloat seekWidth = 27.f;
    CGFloat seekHeight = 27.f;
    CGFloat airplayWidth = 30.f;
    CGFloat airplayHeight = 22.f;
    CGFloat playWidth = 27.f;
    CGFloat playHeight = 27.f;
    CGFloat labelWidth = 30.f;
    
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        //top bar
        CGFloat fullscreenWidth = 34.f;
        CGFloat fullscreenHeight = 44.0f; // self.barHeight;
        CGFloat scaleWidth = 28.f;
        CGFloat scaleHeight = 28.f;
        self.topBar.frame = CGRectMake(0, 0, self.frame.size.width, 64);
        
        UILabel* titleLabel = (UILabel*)[self.topBar viewWithTag:993];
        titleLabel.frame = CGRectMake(50, 42 - 15, self.topBar.frame.size.width - 100, 30);
        
        self.fullscreenButton.frame = CGRectMake(paddingFromBezel, 42 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        self.timeElapsedLabel.frame = CGRectMake(/*self.fullscreenButton.frame.origin.x + self.fullscreenButton.frame.size.width + paddingBetweenButtons*/self.bottomBar.frame.size.width/2 - labelWidth - 5 , 30, labelWidth, self.barHeight);
        ...
        UILabel* label = (UILabel*)[self.bottomBar viewWithTag:990];
        label.frame = CGRectMake(self.bottomBar.frame.size.width/2 - 1, 30, 4, self.barHeight);
        self.timeRemainingLabel.frame = CGRectMake(/*self.scaleButton.frame.origin.x - paddingBetweenButtons - labelWidth*/self.bottomBar.frame.size.width/2 + 3, 30, labelWidth, self.barHeight);
        
        //bottom bar
        self.bottomBar.frame = CGRectMake(0, self.frame.size.height - self.barHeight - 30, self.frame.size.width, self.barHeight + 30);
        self.playPauseButton.frame = CGRectMake(/*self.bottomBar.frame.size.width/2 - playWidth/2*/ 15 + playWidth + paddingBetweenPlaybackButtons, self.barHeight/2 - playHeight/2 + 30, playWidth, playHeight);
        self.seekForwardButton.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width + paddingBetweenPlaybackButtons, self.barHeight/2 - seekHeight/2 + 1.f + 30, seekWidth, seekHeight);
        self.seekBackwardButton.frame = CGRectMake(self.playPauseButton.frame.origin.x - paddingBetweenPlaybackButtons - seekWidth, self.barHeight/2 - seekHeight/2 + 1.f + 30, seekWidth, seekHeight);
        
        self.shareButton.frame = CGRectMake(self.bottomBar.frame.size.width - 80, self.barHeight/2 - 40/2 + 1.f, 40, 40);
        
        UISlider* volumeSlider = (UISlider*)[self.bottomBar viewWithTag:774];
        volumeSlider.frame = CGRectMake(self.seekForwardButton.frame.origin.x + self.seekForwardButton.frame.size.width + 10, self.barHeight/2 - volumeHeight/2, volumeWidth, volumeHeight);
        
        self.volumeView.frame = CGRectMake(self.seekForwardButton.frame.origin.x + self.seekForwardButton.frame.size.width + 10, self.barHeight/2 - volumeHeight/2 + 30, volumeWidth, volumeHeight);
        ...
        self.playPauseButton.frame = CGRectMake(paddingFromBezel, self.barHeight/2 - playHeight/2 + 30, playWidth, playHeight);
        self.timeElapsedLabel.frame = CGRectMake(self.bottomBar.frame.size.width/2 - labelWidth - 5, 30, labelWidth, self.barHeight);
        
        //right side of bottom bar
        CGFloat fullscreenWidth = 28.f;
        CGFloat fullscreenHeight = 44.0f;
        self.fullscreenButton.frame = CGRectMake(paddingFromBezel, 44 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        ...
        self.timeRemainingLabel.frame = CGRectMake(self.bottomBar.frame.size.width/2 + 3, 30, labelWidth, self.barHeight);
    }
    ...
    self.durationSlider.frame = CGRectMake(0/*timeElapsedX + self.timeElapsedLabel.frame.size.width + paddingBetweenLabelsAndSlider*/, /*self.barHeight/2 - sliderHeight/2*/-sliderHeight/2 + 30, /*sliderWidth*/self.bottomBar.frame.size.width, sliderHeight);
