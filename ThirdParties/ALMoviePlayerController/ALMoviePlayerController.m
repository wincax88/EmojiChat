    ...
    if ( (self = [super init]) ) {
        ...
        */
        ...
        if (!_movieBackgroundView) {
            ...
            [_movieBackgroundView setBackgroundColor:[UIColor whiteColor]];
        }
    }
    return self;
}
...
- (void)setControls:(ALMoviePlayerControls *)controls {
    if (_controls != controls) {
        ...
        UITapGestureRecognizer* doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(pauseVideo)];
        doubleTap.numberOfTapsRequired = 2;
        doubleTap.numberOfTouchesRequired = 1;
        //[_controls addGestureRecognizer:doubleTap];
        
        UITapGestureRecognizer* tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showControl)];
        tap.numberOfTapsRequired = 1;
        tap.numberOfTouchesRequired = 1;
        [_controls addGestureRecognizer:tap];
...
- (void)movieFinishedCallback:(NSNotification*)notification {
    ...
    [self stop];
    if (!self.controls.fullscreenButton.touchInside) {
